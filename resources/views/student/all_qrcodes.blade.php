...
<div class="max-w-6xl mx-auto mt-5 bg-white shadow-md rounded-lg overflow-hidden">
    ...
    <div class="flex justify-between items-center mb-4 px-4">
        <form action="{{ route('students.all-qrcodes') }}" method="GET" class="flex w-1/2">
            <input type="text" name="search" class="w-3/4 p-2 border border-gray-300 rounded" value="{{ request()->input('search') }}" placeholder="Search by name or program">
            ...
            <div class="bg-gray-300 shadow-lg rounded-xl overflow-hidden transition-shadow duration-300 hover:shadow-xl student-card">
                <div class="bg-blue-500 p-4 text-center">
                    ...
                    <div class="mb-2 text-white">
                        <h2 class="text-bs font-bold student-name">{{ $student->name }} {{ $student->surname }}</h2>
                        <p class="text-sm student-program">{{ $student->program_of_study }}</p>
                    </div>
                </div>
    ...
    <!-- Pagination links -->
   <div class="mt-6">
    {{ $students->appends(['search' => request()->input('search')])->links('vendor.pagination.tailwind') }}
</div>
